...
].map(([name, nameBn, blood, district, areaBn, area], index) => ({ id: index + 7, name, nameBn, blood, district, area, areaBn, phone: `017000000${String(index + 1).padStart(2, '0')}`, availability: 'যেকোনো সময়', image: `https://randomuser.me/api/portraits/${index % 2 ? 'women' : 'men'}/${index + 11}.jpg`, note: 'রক্তের প্রয়োজনে যোগাযোগ করুন।', rating: 4.8, reviews: 5, verified: true, password: '1234', lastDonation: '2025-10-15', donationHistory: ['2025-10-15', '2025-06-12'] }));
const moreDonorSeeds = [
...
].map(([name, nameBn, blood, district, areaBn, area], index) => ({ id: index + 41, name, nameBn, blood, district, area, areaBn, phone: `018000000${String(index + 1).padStart(2, '0')}`, availability: 'যেকোনো সময়', image: `https://randomuser.me/api/portraits/${index % 2 ? 'women' : 'men'}/${index + 51}.jpg`, note: 'রক্তের প্রয়োজনে যোগাযোগ করুন।', rating: 4.8, reviews: 5, verified: true, password: '1234', lastDonation: '2025-10-15', donationHistory: ['2025-10-15', '2025-06-12'] }));
const allStarterDonors = [...starterDonors, ...additionalDonorSeeds, ...moreDonorSeeds];

function App() {
  const [donors, setDonors] = useState(() => {
    const savedDonors = localStorage.getItem('donors');
    if (!savedDonors) return allStarterDonors;
    const saved = JSON.parse(savedDonors);
    const savedPhones = new Set(saved.map((donor) => donor.phone));
    return [...saved, ...allStarterDonors.filter((donor) => !savedPhones.has(donor.phone))];
  });
  ...
  const [visibleDonorCount, setVisibleDonorCount] = useState(12);
  ...
  useEffect(() => {
    setVisibleDonorCount(12);
  }, [query, districtQuery, blood, district]);
  ...
  return (
    <article className="donor-card">
      ...
      <div className="card-body">
        ...
        <button className="outline-btn" onClick={onOpen}>See more <span>→</span></button>
      </div>
    </article>
  );
}
